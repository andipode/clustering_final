...
def only_full_days(meter_data):
    """
    Reads the input dataframe and deletes all days that still have missing data even after imputation by fill_missing_periods()
    ----------
    meter_data
        The input dataframe
    ...
    -------
    pandas.DataFrame
        The input dataframe without the days with missing values
    """ 
...
def loadto_df_and_dict(mongo_folder, smart_meter_description_csv):
    ...
    -------
    all_meter_data 
        pandas.Dataframe with all available data for all smart meters
    dict
        dictionary with key:meter ID -> value:pandas.Dataframe with all data for this smart meter ID
    meter_description
        pandas.Dataframe edition of the smart_meter_description_csv file
    """
...
def fill_missing_periods(meter_data, filling_window: int, fill_night: bool, id, meter_description, resolution):
    """
    Loads data for a specific smart meter and fills all missing periods smaller than filling_window and possibly 
    missing periods during the night (depending on the value of fill_night) using linear interpolation.
    ----------
    meter_data
        pandas.Dataframe with all the readings for one smart meter
    filling_window
        (hours) all periods shorter than filling_window hours will be filled
    fill_night
        if True all missing periods in the 00:00 to 06:00 interval will be filled
    id
        smart meter id to read info from the smart_meter_description dataframe
    meter_description
        smart_meter_description dataframe
    sampling_period
        dataset resolution in min

    Returns
    -------
    meter_data
        pandas.Dataframe like the input dataframe but with missing periods filled
    """
...
def fix_scaling(dict):
    """
    some Mongo data had scaling issues (measurement scale changed at certain points). This is an attempt to fix those issues.
    -----
    dict
        dictionary created in loadto_df_and_dict
...
def data_preprocessing(all_meter_data, dict, meter_description, resolution):
    """
    Pipeline for all the preprocessing done on Mongo data. At the end it saves the processed data in a csv file (clean_mongo.csv)
    ----------
    all_meter_data, dict, meter_description
        as created by loadto_df_and_dict
    sampling_period
        dataset resolution in min
